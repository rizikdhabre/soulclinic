...
describe("customer identity lookup boundary", () => {
  beforeEach(() => {
    getCollectionMock.mockReset();
    getCollectionMock.mockRejectedValue(new Error("Unexpected database access"));
  ...
  it("does not expose customer fields in a new OTP challenge", async () => {
    ...
    const challengeStore = { create: vi.fn().mockResolvedValue({ _id: "challenge-id" }) };
    ...
    const result = await createOtpChallenge(
      ...
      {
        env: { NODE_ENV: "production", CUSTOMER_SESSION_SECRET: "s".repeat(48) },
    ...
    expect(getCollectionMock).not.toHaveBeenCalled();
    expect(challengeStore.create).toHaveBeenCalledWith(expect.objectContaining({
      phone: "+972521234567", purpose: "booking", challengeTokenHash: hashBearerToken(result.challengeToken),
    }));
    expect(JSON.stringify(challengeStore.create.mock.calls)).not.toContain(result.challengeToken);
  });

  it("does not read a profile when Twilio rejects completion", async () => {
    const now = new Date("2026-08-23T12:00:00.000Z");
    const usersData = { findOne: vi.fn() };
    const issueBookingGrant = vi.fn();
    const challengeToken = "a".repeat(43);
    const verificationSid = `VE${"a".repeat(32)}`;
    const verifyTwilioCode = vi.fn().mockResolvedValue({
      sid: verificationSid, to: "+972521234567", status: "pending",
    });
    const collection = new MemoryMongoCollection([{
      _id: "challenge-id", challengeTokenHash: hashBearerToken(challengeToken),
      phone: "+972521234567", purpose: "booking", provider: "twilio", status: "sent", verificationSid,
      createdAt: new Date("2026-08-23T11:59:00.000Z"), expiresAt: new Date("2026-08-23T12:10:00.000Z"),
    }]);
    ...
    expect(verifyTwilioCode).toHaveBeenCalledExactlyOnceWith("+972521234567", "654321", verificationSid);
    expect(usersData.findOne).not.toHaveBeenCalled();
