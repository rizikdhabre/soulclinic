...
export function openOtpReceipt(receipt, { challenge, operation, now, env = process.env }) {
  try {
    ...
    if (
      !["send", "verify"].includes(operation) || result.operation !== operation || result.challengeTokenHash !== challenge.challengeTokenHash ||
      ...
      !/^VE[0-9a-f]{32}$/i.test(result.verificationSid) ||
      ...
      (operation === "verify" && result.expiresAt !== result.observedAt + OTP_GRANT_TTL_MS) ||
      (operation === "verify" && challenge.purpose === "login" && (!Number.isSafeInteger(result.sessionTtlSeconds) || result.sessionTtlSeconds <= 0)) ||
