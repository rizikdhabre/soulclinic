...
const MissedAppointmentsList = ({ isOpen, onToggleAttendance }) => {
  ...
  const fetchBatch = async (nextOffset) => {
    ...
    try {
      ...
      setError("تعذر تحميل المواعيد الفائتة");
    } finally {
      if (!completed) {
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Switch
              ...
              onCheckedChange={(checked) => {
                onToggleAttendance(item.appointmentId, checked);

                if (!checked) return;

                setItems((prev) =>
                  prev.filter(
                    (currentItem) => currentItem.appointmentId !== item.appointmentId,
                  ),
                );
